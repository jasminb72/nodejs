...
class ModuleData {
    ...
    modifierContact(contactModifie) {
        // 1) recherche du contact modifié dans la base
        // 2) contact de la base [paramètres] = contact modifié [paramètres]
        for (contactBase of this.contacts) {
            if (contactBase.id == contactModifie.id) {
                for (parametre in contactBase) {
                    contactBase[parametre] = contactModifie[parametre]
                }
            }
        }
    }
